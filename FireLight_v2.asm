...
SET_EMER_DURATION_END:
	LDI	CNT0_EMERGENCY,	00H	;清"本次应急时长"计数器
	LDI	CNT1_EMERGENCY,	00H
	LDI	CNT2_EMERGENCY,	00H
	RTNI
...
SCS_LESS_3S:
	...
	LDI	TBR,		1110B
	AND	SELF_STATE
	BAZ	SELF_CHK_STATE_END	;如果当前不是"月检"或"年检"或"模拟停电"状态，则跳转
...
SCP_MONTH_1S:
	...
	SBI	CNT0_EMERGENCY,	08H	;和120秒比较(0x078)
	LDI	TBR,		07H
	SBC	CNT1_EMERGENCY
	LDI	TBR,		00H
	SBC	CNT2_EMERGENCY
...
SCP_ARRIVE_120S:
	LDA	FIXED_SELF_CHK
	BA0	SCP_MONTH_NO_EXIT	;如果自检过程中，检测到有故障，则即使到了120秒，也不能自动退出月检状态
...
SCP_MONTH_NO_EXIT:
	LDI	TBR,		08H	;将应急时长计数器与30分钟(1800s -> 0x708)比较，如果到了30分钟了，就不必再累加了。
	SUB	CNT0_EMERGENCY
	LDI	TBR,		00H
	SBC	CNT1_EMERGENCY
	LDI	TBR,		07H
	...
	BC	SCP_BAT_EXHAUSTED_CHK	;应急时长已超过30分钟，计数器无需再累加，跳转

	ADIM	CNT0_EMERGENCY,	01H	;应急时长计数器加1
	LDI	TBR,		00H
	ADCM	CNT1_EMERGENCY
	LDI	TBR,		00H
	ADCM	CNT2_EMERGENCY

SCP_BAT_EXHAUSTED_CHK:
	ADI	BAT_EXHAUSTED,	01H
	BA0	SELF_CHK_PROCESS_END
